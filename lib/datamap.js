...
// ความยาว "เชิงสายตา" — ไม่นับสระบน/ล่าง+วรรณยุกต์ไทย (กว้าง ~0) จึงไม่ wrap เร็วเกินจริง
const VCOMB=/[ัิ-ฺ็-๎]/g;
function vlen(s){ return s.replace(VCOMB,'').length; }
...
// แก้วงเล็บหลุดบรรทัดที่ "ขอบบรรทัด" โดยตรง (ครอบคลุมทุกเคส: สั้น/ยาว/ซ้อน/ไม่ปิด)
//  "(" ค้างท้ายบรรทัด → ดันลงต้นบรรทัดถัดไป · ")" ขึ้นต้นบรรทัด → ดึงขึ้นท้ายบรรทัดก่อนหน้า
function dmDeorphan(lines){
  for(let i=0;i<lines.length-1;i++){
    const m=lines[i].match(/\(+\s*$/);
    if(m){ lines[i]=lines[i].slice(0,m.index).replace(/\s+$/,''); lines[i+1]=m[0].replace(/\s+/g,'')+lines[i+1].replace(/^\s+/,''); }
  }
  for(let i=1;i<lines.length;i++){
    const m=lines[i].match(/^\s*\)+/);
    if(m){ lines[i-1]=lines[i-1]+m[0].replace(/\s+/g,''); lines[i]=lines[i].slice(m[0].length).replace(/^\s+/,''); }
  }
  const r=lines.filter(l=>l!=='');
  return r.length?r:[''];
}
function dmWrap(s,n){
  ...
  for(let w of words){
    while(vlen(w)>n){
      ...
      let part='';
      for(const g of gr){ if(vlen(part+g)>n) break; part+=g; }
      if(!part){ part=gr[0]||w.slice(0,n); }   // อย่างน้อย 1 grapheme — ไม่ตัดกลาง cluster
      if(line){ out.push(line.replace(/\s+$/,'')); line=''; }
      out.push(part); w=w.slice(part.length);
    }
    if(vlen(line+w)>n){ if(line.trim())out.push(line.replace(/\s+$/,'')); line=w.replace(/^\s+/,''); }
    else line+=w;
  }
  if(line.trim()) out.push(line.replace(/\s+$/,''));
  return dmDeorphan(out.length?out:['']);
}
...
// sy = จุดเริ่มแนวตั้ง (ถ้าระบุ) — เว้นช่องใต้กล่องไว้วาง note ระหว่างกล่องกับเส้น
function dmDownArr(a,b,sy){ const x1=a.x+a.w/2,y1=(sy!=null?sy:a.y+a.h),x2=b.x+b.w/2,y2=b.y,my=(y1+y2)/2;
...
function dmNoteEl(x,y,text){
  ...
  const lines=dmWrap(text,80);
  const tw=Math.max.apply(null, lines.map(l=>vlen(l)))*6.6;   // กว้างเชิงสายตา
  // halo ขาวบางรอบตัวอักษร (paint-order: stroke วาดก่อน fill) — อ่านง่ายเหนือเส้น
  // โดยเส้นลูกศรยังต่อเนื่องลอดระหว่างตัวอักษร (ไม่ถูกพื้นหลังทึบบังจนหาย)
  let t='<text x="'+x+'" y="'+y+'" font-size="11" fill="#c0392b" paint-order="stroke" stroke="#ffffff" stroke-width="2.6" stroke-linejoin="round">';
...
export function buildMapSVG(r){
  ...
  NOTE.push(dmNoteEl(ent.x, ent.y+ent.h+15, 'เมนู ระยะเวลาในการเก็บรักษาข้อมูล → ประเภทการเก็บรักษา'));
  const fmtItems = formats.length?more(formats,5):['(ยังไม่ระบุ)'];
  const fmtGrid=dmGrid(fmtItems, ent.x, ent.y+ent.h+58, 160, fmtItems.length, 'box');
  fmtGrid.nodes.forEach(n=>{ N.push(n); C.push(dmDownArr(ent,n, ent.y+ent.h+28)); });
  ...
  if(s6items.length){
    ...
    shown.forEach((it,j)=>{
      ...
      rb.x=ent.x+j*(w+gap); rb.y=retY; N.push(rb); C.push(dmDownArr(fmtGrid.nodes[j]||ent,rb, retY-22));
    });
    ...
    const rb=dmNode('box','การเก็บรักษา',['ไม่มีการจัดเก็บ'],160); rb.x=ent.x; rb.y=retY; N.push(rb); C.push(dmDownArr(fmtGrid.nodes[0]||ent,rb, retY-22));
  }
  NOTE.push(dmNoteEl(ent.x, retY-35, '→ แต่ละประเภท: เริ่มนับจากระยะเวลา → ระยะเวลาที่เก็บรักษา'));
  ...
  if(use||dis||trx){
    ...
    if(use) C.push(dmVArr(use.x+use.w/2, trunkY, use.y-26));   // จบเหนือ note (note ระหว่างเส้นกับกล่อง)
    if(dis) C.push(dmVArr(dis.x+dis.w/2, trunkY, dis.y-26));
