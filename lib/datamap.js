...
function dmNoteEl(x,y,text){
  ...
  // พื้นหลังขาวใต้ตัวอักษร — mask เส้นลูกศรที่พาดผ่าน ให้ note อยู่ "เหนือเส้น" ชัดเจน
  const bg='<rect x="'+(x-3)+'" y="'+(y-10)+'" width="'+(tw+6)+'" height="'+((lines.length-1)*13+14)+'" fill="#ffffff"/>';
  let t='<text x="'+x+'" y="'+y+'" font-size="11" fill="#c0392b">';
  lines.forEach((l,i)=> t+='<tspan x="'+x+'" '+(i?'dy="13"':'')+'>'+esc(l)+'</tspan>');
  t+='</text>';
  return { svg:bg+t, r:x+tw, b:y+(lines.length-1)*13+4 };
}
