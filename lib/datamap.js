...
function dmNoteEl(x,y,text){
  const lines=dmWrap(text,46); let t='<text x="'+x+'" y="'+y+'" font-size="11" fill="#c0392b">';
...
export function buildMapSVG(r){
  ...
  const discloseOn=s4.disclose==='มีการเปิดเผย', recipients=dmArr(s4.recipient);
  ...
  const svg='<svg xmlns="http://www.w3.org/2000/svg" width="'+frameW+'" height="'+frameH+'" viewBox="'+vbX+' 0 '+frameW+' '+frameH
    ...
    +C.join('')+N.map(dmDraw).join('')+NOTE.join('')+'</svg>';
