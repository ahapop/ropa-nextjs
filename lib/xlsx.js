...
/* ---- generic multi-sheet builder (สำหรับรายงานสรุป/worklist) ----
   sheets: [{ name, rows:[[cell,...],...], headerRows?:1 }] */
function _genSheetXml(rows, headerRows){
  let body='';
  rows.forEach((cells,ri)=>{
    const r1=ri+1, style = ri<headerRows ? 2 : 3;
    let cs=''; cells.forEach((v,ci)=>{ cs += _xlCell(ci, r1, (v==null?'':String(v)), style); });
    body += `<row r="${r1}">${cs}</row>`;
  });
  const maxCols = rows.reduce((m,r)=>Math.max(m, r.length), 1);
  const colW='<cols>'+Array.from({length:maxCols},(_,i)=>`<col min="${i+1}" max="${i+1}" width="${i===0?34:20}" customWidth="1"/>`).join('')+'</cols>';
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">${colW}<sheetData>${body}</sheetData></worksheet>`;
}
export function buildSheetsXlsx(sheets){
  ...
  const styRid = sheets.length+1;
  const wbr = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'+
    sheets.map((s,i)=>`<Relationship Id="rId${i+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${i+1}.xml"/>`).join('')+
    `<Relationship Id="rId${styRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`;
  return _zip([
    ...
    {name:'xl/styles.xml',data:_u8(_XL_STY)},
    ...sheets.map((s,i)=>({ name:`xl/worksheets/sheet${i+1}.xml`, data:_u8(_genSheetXml(s.rows, s.headerRows||1)) })),
  ]);
}
