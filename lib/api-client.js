...
export const api = {
  ...
  listRecordsFull: (range) => {                                              // full (export/dashboard) + ช่วงเวลา
    let q = "/api/records?full=1";
    if(range && range.from != null) q += "&from=" + range.from;
    if(range && range.to   != null) q += "&to="   + range.to;
    return jfetch(q).then(d => d.records);
  },
  ...
  seedFull: () => jfetch("/api/seed", { method: "POST" }).then(d => d.count),   // ข้อมูลจำลองครบทุกบริษัท (admin)
  // snapshots (trend)
  snapshots: (days=90) => jfetch("/api/snapshot?days=" + days).then(d => d.snapshots),
  captureSnapshot: () => jfetch("/api/snapshot", { method: "POST" }),
  // users (admin)
