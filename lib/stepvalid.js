...
function requiredFids(stepKey, rec){
  ...
  switch(stepKey){
    ...
    case 's4': {
      out.push('s4.disclose');
      if(g('s4.disclose')==='มีการเปิดเผย'){
        out.push('s4.recipient','s4.recipientDetail','s4.purpose','s4.contract','s4.method','s4.dpa');
        addOther('s4.recipient', MASTER.externalRecipients, 'chk');
        addOther('s4.method', MASTER.disclosureMethods, 'chk');
      }
      break;
    }
    case 's5': {
