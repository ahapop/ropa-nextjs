...
// เกณฑ์ความครบของ 1 Record ใน Step 4 (การเปิดเผย) — 4.1,4.2,4.4,4.5,4.6,4.7 (4.3 สถานะไม่บังคับ)
export function s4ItemOk(it){
  if(!it) return false;
  const r=(it.recipient||'').toString().trim(), detail=(it.recipientDetail||'').toString().trim();
  const purpose=(it.purpose||'').toString().trim(), contract=(it.contract||'').toString().trim(), dpa=(it.dpa||'').toString().trim();
  const method=Array.isArray(it.method)&&it.method.length>0;
  return !!(r && detail && purpose && contract && method && dpa);
}
...
export function isStepComplete(r, i){
  ...
  const checks = {
    ...
    s4: () => { const v=get('s4.disclose'); if(!v) return false; if(v==='ไม่มีการเปิดเผย') return true;
      const items=get('s4.items')||[]; if(!items.length) return false;
      const recips=items.map(it=>it&&it.recipient).filter(Boolean);
      return items.every(s4ItemOk)&&new Set(recips).size===recips.length; },
...
export const EXTRA_RULES = {
  ...
  s4(r){
    if(getDeep(r,'s4.disclose')!=='มีการเปิดเผย') return null;
    const items=getDeep(r,'s4.items')||[];
    if(!items.length) return {msg:'ต้องเพิ่มผู้รับข้อมูล (การเปิดเผย) อย่างน้อย 1 รายการ'};
    const recips=items.map(it=>it&&it.recipient).filter(Boolean);
    if(new Set(recips).size!==recips.length) return {msg:'ผู้รับข้อมูล (4.1) ซ้ำกัน — แต่ละรายการต้องเป็นคนละผู้รับ'};
    const bad=items.findIndex(it=>!s4ItemOk(it));
    if(bad>=0) return {msg:`รายการผู้รับข้อมูลลำดับที่ ${bad+1} กรอกไม่ครบ — ต้องมี 4.1 ผู้รับ, 4.2 รายละเอียด, 4.4 วัตถุประสงค์, 4.5 สัญญา, 4.6 วิธีส่ง, 4.7 DPA`};
    return null;
  },
  s6(r){
