...
export async function listRecordsFull({ all, userId }){
  const rows = all
...
export async function listSummaries({ all, userId }){
  const rows = all
    ? await sql`SELECT id, company, org, status, complete, updated_ts AS uts, activity, activity_other AS ao, recorder_name AS rn, updated_at AS ua
                FROM records ORDER BY updated_ts DESC`
    : await sql`SELECT id, company, org, status, complete, updated_ts AS uts, activity, activity_other AS ao, recorder_name AS rn, updated_at AS ua
                FROM records WHERE user_id=${userId} ORDER BY updated_ts DESC`;
  return rows.map(r => ({
...
export async function getRecord(id){
  const r = await sql`SELECT data, user_id FROM records WHERE id=${id} LIMIT 1`;
  return r[0] ? { data: r[0].data, ownerId: r[0].user_id } : null;
}
