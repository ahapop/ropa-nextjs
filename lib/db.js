...
export async function insertRecordsWithOwners(pairs){
  ...
  for(let i=0;i<pairs.length;i+=B){
    ...
    chunk.forEach(({rec, owner}, j)=>{
      const c = cols(rec); const b = j*14;
      const cat = rec.createdTs ? new Date(rec.createdTs).toISOString() : null;   // วันสร้าง (ถ้าระบุ) ไม่งั้น now()
      ph.push(`($${b+1},$${b+2},$${b+3},$${b+4},$${b+5},$${b+6},$${b+7},$${b+8},$${b+9},$${b+10},$${b+11},$${b+12},$${b+13}::jsonb,COALESCE($${b+14}::timestamptz,now()))`);
      vals.push(rec.id, owner, c.company, c.org, c.status, c.updated_ts, c.complete, c.activity, c.activity_other, c.recorder_name, c.updated_at, c.department, JSON.stringify(rec), cat);
    });
    await rawQuery(
      `INSERT INTO records (id,user_id,company,org,status,updated_ts,complete,activity,activity_other,recorder_name,updated_at,department,data,created_at) VALUES ${ph.join(',')}
...
export async function seedAllCompanies(perUser = 5){
  ...
  // สุ่มวันเดือนปี ตั้งแต่ 1 ม.ค. 2024 ถึงปัจจุบัน (ให้เห็นความหลากหลายของวันที่)
  const START = Date.parse("2024-01-01T00:00:00"), NOW = Date.now();
  const randTs = () => START + Math.floor(Math.random()*(NOW - START));
  const thDate = (ms) => new Date(ms).toLocaleString("th-TH", { dateStyle:"medium", timeStyle:"short" });
  ...
  for(const u of users){
    ...
    for(const r of makeDummyByDept(perUser, org, dept)){
      r.company = company;
      const ts = randTs();                              // วันสุ่ม 2024 → ปัจจุบัน
      r.updatedTs = ts; r.createdTs = ts; r.updatedAt = thDate(ts); r.createdAt = thDate(ts);
