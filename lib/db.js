...
// WHERE clause ตาม scope (all / division / own)
...
export async function listSummaries(scope){
  ...
  const rows = await rawQuery(
    `SELECT id, company, org, status, complete, updated_ts AS uts, activity, activity_other AS ao, recorder_name AS rn, updated_at AS ua
     FROM records ${w.sql} ORDER BY updated_ts DESC`, w.params);
  return rows.map(r => ({
    id: r.id, company: r.company, status: r.status, complete: r.complete,
...
export async function getRecord(id){
  const r = await sql`SELECT rc.data, rc.user_id, u.division AS owner_division
                      FROM records rc JOIN users u ON u.id=rc.user_id WHERE rc.id=${id} LIMIT 1`;
  return r[0] ? { data: r[0].data, ownerId: r[0].user_id, ownerDivision: r[0].owner_division } : null;
}
// ข้อมูลสิทธิ์ของ record (owner + ฝ่ายของ owner) — ใช้เช็คการเข้าถึงโดยไม่อ่าน JSONB
export async function getRecordAccessInfo(id){
  const r = await sql`SELECT rc.user_id, u.division AS owner_division
                      FROM records rc JOIN users u ON u.id=rc.user_id WHERE rc.id=${id} LIMIT 1`;
  return r[0] ? { ownerId: r[0].user_id, ownerDivision: r[0].owner_division } : null;
}
export async function upsertRecord(rec, ownerId){
  const c = cols(rec);
  await sql`INSERT INTO records (id,user_id,company,org,status,updated_ts,complete,activity,activity_other,recorder_name,updated_at,data)
            VALUES (${rec.id},${ownerId},${c.company},${c.org},${c.status},${c.updated_ts},${c.complete},${c.activity},${c.activity_other},${c.recorder_name},${c.updated_at},${JSON.stringify(rec)}::jsonb)
            ON CONFLICT (id) DO UPDATE SET
              ...
              recorder_name=EXCLUDED.recorder_name, updated_at=EXCLUDED.updated_at, data=EXCLUDED.data`;
}
...
export async function insertRecordsWithOwners(pairs){
  ...
  for(let i=0;i<pairs.length;i+=B){
    ...
    chunk.forEach(({rec, owner}, j)=>{
      const c = cols(rec); const b = j*12;
      ph.push(`($${b+1},$${b+2},$${b+3},$${b+4},$${b+5},$${b+6},$${b+7},$${b+8},$${b+9},$${b+10},$${b+11},$${b+12}::jsonb)`);
      vals.push(rec.id, owner, c.company, c.org, c.status, c.updated_ts, c.complete, c.activity, c.activity_other, c.recorder_name, c.updated_at, JSON.stringify(rec));
    });
    await rawQuery(
      `INSERT INTO records (id,user_id,company,org,status,updated_ts,complete,activity,activity_other,recorder_name,updated_at,data) VALUES ${ph.join(',')}
       ON CONFLICT (id) DO UPDATE SET company=EXCLUDED.company,org=EXCLUDED.org,status=EXCLUDED.status,updated_ts=EXCLUDED.updated_ts,complete=EXCLUDED.complete,activity=EXCLUDED.activity,activity_other=EXCLUDED.activity_other,recorder_name=EXCLUDED.recorder_name,updated_at=EXCLUDED.updated_at,data=EXCLUDED.data`,
      vals
    );
  }
  return pairs.length;
