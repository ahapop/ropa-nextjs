...
export async function insertRecordsWithOwners(pairs){
  ...
  for(let i=0;i<pairs.length;i+=B){
    ...
    chunk.forEach(({rec, owner}, j)=>{
      const c = cols(rec); const b = j*7;
      ph.push(`($${b+1},$${b+2},$${b+3},$${b+4},$${b+5},$${b+6},$${b+7}::jsonb)`);
      vals.push(rec.id, owner, c.company, c.org, c.status, c.updated_ts, JSON.stringify(rec));
    });
    await rawQuery(
      `INSERT INTO records (id,user_id,company,org,status,updated_ts,data) VALUES ${ph.join(',')}
       ON CONFLICT (id) DO UPDATE SET company=EXCLUDED.company,org=EXCLUDED.org,status=EXCLUDED.status,updated_ts=EXCLUDED.updated_ts,data=EXCLUDED.data`,
      vals
    );
  }
  return pairs.length;
