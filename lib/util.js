...
export function migrateS4(rec){
  ...
  if(s.items.length===0 && hasFlat){
    // legacy 4.1 เก็บผู้รับเป็น array (multi-select) → สร้าง 1 item ต่อ 1 ผู้รับ (เหมือน migrateS3)
    const recips = (Array.isArray(s.recipient) && s.recipient.length) ? s.recipient : [Array.isArray(s.recipient) ? '' : (s.recipient||'')];
    const status0 = Array.isArray(s.status) ? (s.status[0]||'') : (s.status||'');
    const mk = r => ({ recipient:r, recipientDetail:s.recipientDetail||'', recipientFile:null, status:status0, purpose:s.purpose||'', contract:s.contract||'', method:Array.isArray(s.method)?s.method.slice():[], dpa:s.dpa||'' });
    s.items = recips.filter(r=>r!=null && String(r).trim()!=='').map(mk);
    if(!s.items.length) s.items = [mk('')];
  }
  return rec;
}
