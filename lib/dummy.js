/* DUMMY DATA — สร้างข้อมูลตัวอย่างที่กรอกครบทุก Block — ported จาก ropa_v32.html
   makeDummyRecords(n)        : n รายการ org สุ่ม
   makeDummyByOrg(perOrg,orgs): perOrg รายการต่อหน่วยงาน (ฝ่าย/ส่วน) แต่ละตัว */
import { MASTER, DIVISIONS, SECTIONS, sectionsFor } from "./master";
...
function buildRecord(ts, i, orgOverride, departmentOverride){
  ...
  return {
    id:uid(), status:'done', createdAt:ts, updatedAt:ts, updatedTs:Date.now()-i*60000, company, department: departmentOverride || '',
    recorder:{firstName:fn, lastName:ln, position:pick(positions), phone:'08'+Math.floor(10000000+rnd()*89999999), division, section, department: departmentOverride || ''},
    s1:{org: orgOverride || pick(MASTER.orgStructure), activity,
...
export function makeDummyRecords(n){
  n=n||100; const ts=nowStr(); const out=[];
  for(let i=0;i<n;i++) out.unshift(buildRecord(ts, i));
  return out;
}

// perOrg รายการ ต่อ 1 หน่วยงาน — ค่าเริ่มต้น: ทุกฝ่าย + ทุกส่วน
export function makeDummyByOrg(perOrg, orgs){
  perOrg = perOrg || 20;
  orgs = orgs || [...DIVISIONS, ...SECTIONS];
  const ts = nowStr(); const out = []; let i = 0;
  for(const org of orgs){ for(let k=0;k<perOrg;k++){ out.push(buildRecord(ts, i++, org)); } }
  return out;
}

// perDept รายการ ของ 1 แผนก (อยู่ใต้ sectionOrg) — record มี s1.org=ส่วน, department=ชื่อแผนก
export function makeDummyByDept(perDept, sectionOrg, deptName){
  perDept = perDept || 20;
  const ts = nowStr(); const out = [];
  for(let k=0;k<perDept;k++) out.push(buildRecord(ts, k, sectionOrg, deptName));
  return out;
}
